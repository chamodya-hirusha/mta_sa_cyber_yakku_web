...
export default function ProductCarousel({ title, icon, addToCart }) {
  const [isHovered, setIsHovered] = React.useState(false);

  // ✅ Local products array (no conflict with props)
  const products = [
    {
      id: 1,
  ...
  return (
    <div className="container py-3 tablet:py-4 desktop:py-5 w-full px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-6 md:py-8">
      {/* Header Section */}
      <motion.div
      ...
      >
        ...
        {/* <button className="sm:hidden  items-center gap-2 px-3 sm:px-4 py-1.5 sm:py-2 text-xs sm:text-sm text-purple-300 hover:text-white bg-purple-500/10 hover:bg-purple-500/20 border border-purple-500/30 hover:border-purple-400/50 rounded-lg transition-all duration-300">
          <span>View All</span>
          <svg
            className="w-3 h-3"
            fill="none"
          ...
          >
            ...
            >
              {products.map((product, index) => (
                <CarouselItem
                  key={product.id}
                  className="basis-[25%] basis-h-[40%] sm:basis-1/2 md:basis-1/2 lg:basis-1/4 xl:basis-1/5 2xl:basis-1/6 pl-2 sm:pl-3 md:pl-4"
                >
                  <motion.div
      ...
      <button className="sm:hidden w-full mt-4 flex items-center justify-center gap-2 px-4 py-3 text-sm text-purple-300 bg-purple-500/10 border border-purple-500/30 rounded-lg transition-all duration-300 active:scale-95">
